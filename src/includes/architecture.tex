...
\begin{itemize}[nosep]
    ...
    \item Enhanced test suite: 181 tests across the repository (some runs may skip two optional tests)
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item Verification CLI can accept either OTS or TSA proofs for timestamp attestation
\end{itemize}
...
Because Track1 pods presently emit low-rate scalar telemetry (no images, no high-rate biosignals), dedicating energy budget to watermarking would provide no security benefit and would collide with the 1.4~Wh/day envelope. These techniques remain in the backlog for future hardware revisions and are documented in ADR-XXX.
