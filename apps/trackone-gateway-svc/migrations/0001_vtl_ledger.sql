-- Owned and applied by the trackone-gateway-svc application.
CREATE TABLE IF NOT EXISTS trackone_vtl_ledger_state (
...
CREATE TABLE IF NOT EXISTS trackone_vtl_open_record (
    ledger_id text NOT NULL REFERENCES trackone_vtl_ledger_state(ledger_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS trackone_vtl_sealed_segment (
    ledger_id text NOT NULL REFERENCES trackone_vtl_ledger_state(ledger_id),
    ...
    tsa_status text NOT NULL DEFAULT 'queued' CHECK (tsa_status IN ('queued', 'verified')),
    tsa_response bytea,
    PRIMARY KEY (ledger_id, segment_number)
);

CREATE TABLE IF NOT EXISTS trackone_vtl_sealed_record (
...
CREATE TABLE IF NOT EXISTS trackone_vtl_idempotency (
    ledger_id text NOT NULL REFERENCES trackone_vtl_ledger_state(ledger_id) ON DELETE CASCADE,
    idempotency_key text NOT NULL CHECK (length(idempotency_key) BETWEEN 1 AND 255),
    request_sha256 text NOT NULL CHECK (request_sha256 ~ '^[0-9a-f]{64}$'),
    admitted_segment_numbers text[] NOT NULL,
