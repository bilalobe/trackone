...
- Alpha.10 work in flight:

  - Demo/device state split:
  ...
  - Pipeline manifest contract:
    - `day/<date>.pipeline-manifest.json` is now schema-backed and carries locked `disclosure_class`, `commitment_profile_id`, artifact path+digest entries, and executed/skipped checks
    - `verify_cli.py` validates that manifest when present
  - SensorThings projection contract:
