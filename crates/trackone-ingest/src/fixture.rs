...
#[cfg(feature = "xchacha")]
use crate::{
    DeviceMaterial, encode_fact_postcard, framed_aad, framed_nonce, legacy_dev_id_from_pod_id,
};
#[cfg(feature = "xchacha")]
...
#[cfg(feature = "xchacha")]
use trackone_core::{
    AEAD_NONCE_LEN, AEAD_TAG_LEN, EnvFact, Fact, FactKind, FactPayload, MAX_FACT_LEN, SampleType,
};
...
pub fn emit_fixture(
    ...
    if msg_type != crate::FRAMED_FACT_MSG_TYPE {
        return Err(FixtureError::Reject(RejectReason::UnsupportedMessageType));
    }
    if legacy_dev_id_from_pod_id(device.expected_pod_id()) != dev_id {
        return Err(FixtureError::Reject(RejectReason::HeaderDeviceIdMismatch));
    }

    let observed_at = pod_time.unwrap_or(1_700_000_000 + i64::from(fc));
    let fact = Fact {
        pod_id: device.expected_pod_id(),
        fc: u64::from(fc),
        ingest_time: 0,
        pod_time: Some(observed_at),
        kind: FactKind::Env,
        payload: FactPayload::Env(
            EnvFact::instant(SampleType::AmbientAirTemperature, observed_at, 20.0)
                .map_err(|_| FixtureError::Reject(RejectReason::InvalidFact))?,
        ),
    };
