//! Reusable, Rust-native OpenTimestamps proof and metadata verification.
...
/// Return the SHA-256 digest used as the initial OpenTimestamps message.
pub fn hash_for_ots_native(artifact: &[u8]) -> [u8; 32] {
    sha256_digest(artifact)
}

/// Describe the operations contained in a supported detached proof.
pub fn describe_ots_proof_native(
    ots_path: &Path,
    expected_artifact_sha: Option<&str>,
) -> Result<Vec<String>, &'static str> {
    describe_detached_ots_proof_impl(ots_path, expected_artifact_sha).map_err(|error| match error {
