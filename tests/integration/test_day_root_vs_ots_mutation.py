...
- The Merkle day_root depends only on the content of the day artifact (e.g. day/2025-10-07.cbor).
...
def test_day_root_ignores_day_bin_mutation_for_now(
    ...
    """Verify that mutating day.cbor fails validation when OTS metadata is present.

    The built_day_artifacts fixture always creates an OTS meta sidecar that
    includes the artifact SHA256. After mutating day.cbor, verify_cli parses
    and validates the day.cbor content, then checks the artifact hash against
